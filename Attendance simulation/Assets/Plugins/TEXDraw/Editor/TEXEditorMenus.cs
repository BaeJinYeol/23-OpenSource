...
Felis et imperdiet euismod, purus ipsum pretium metus, in lacinia nulla nisl eget sapien. Donec ut est in lectus consequat consequat. Etiam eget dui. Aliquam erat volutpat. Sed at lorem in nunc porta tristique.";
...
#endif
                    EditorUtility.SetDirty(TEXPreference.main.defaultMaterial);
                    EditorPrefs.DeleteKey("TEXDraw_WillProceedTMP");
                };
            }

            if (EditorPrefs.GetBool("TEXDraw_WillProceedUIE", false))
            {
                EditorApplication.delayCall += delegate ()
                {
                    TEXPreference.Initialize();
#if TEXDRAW_UIE
                TEXPreference.main.GetDefaultMaterialForUIE();
#endif
                    EditorPrefs.DeleteKey("TEXDraw_WillProceedUIE");
                };
            }
        }
        ...
        private const string TEXDRAW_UIE = "TEXDRAW_UIE";
        private const string TEXDRAW_DEBUG = "TEXDRAW_DEBUG";
        ...
        [MenuItem("Tools/TEXDraw/Enable InputSystem Integration", false, 101)]
        ...
        [MenuItem("Tools/TEXDraw/Enable UIElements Integration", false, 102)]
        public static void SetUIElementsIntegration()
        {
            if (IsDefined(TEXDRAW_UIE))
            {
                if (EditorUtility.DisplayDialog("Removing TEXDRAW_UIE",
                     "This will unset UI Elements support. Make sure no UI Documents are using TEXDraw before continue.",
                     "Proceed", "Cancel"))
                {
                    EditorPrefs.SetBool("TEXDraw_WillProceedUIE", true);
                    SetDefined(TEXDRAW_UIE, false, true);
                }
            }
            else
            {
                if (EditorUtility.DisplayDialog("Add TEXDRAW_UIE integration for all platform",
                     "This will enable support for TEXDraw UE (UI Elements component). Be advised that it has a limited support currently.",
                     "Proceed", "Cancel"))
                {
                    TEXPreference.Initialize();
                    EditorPrefs.SetBool("TEXDraw_WillProceedUIE", true);
                    SetDefined(TEXDRAW_UIE, true, true);
                }
            }
        }

        [MenuItem("Tools/TEXDraw/Enable UIElements Integration", true, 102)]
        private static bool ValidateUIElementsIntegration()
        {
            Menu.SetChecked("Tools/TEXDraw/Enable UIElements Integration", IsDefined(TEXDRAW_UIE));
            return true;
        }

        [MenuItem("Tools/TEXDraw/Enable Debug Mode", false, 200)]
        public static void SetDebugMode()
        {
            if (IsDefined(TEXDRAW_DEBUG))
            {
                SetDefined(TEXDRAW_DEBUG, false, true);
            }
            else
            {
                if (EditorUtility.DisplayDialog("Add TEXDRAW_DEBUG",
                     "This will enable some debugging features, useful for patching TEXDraw's code.",
                     "Proceed", "Cancel"))
                {
                    SetDefined(TEXDRAW_DEBUG, true, false);
                }
            }
        }

        [MenuItem("Tools/TEXDraw/Enable Debug Mode", true, 200)]
        private static bool ValidateDebugMode()
        {
            Menu.SetChecked("Tools/TEXDraw/Enable Debug Mode", IsDefined(TEXDRAW_DEBUG));
            return true;
        }
        ...
        {
            SetDefined(defineName, enable, GetCurrentTarget());
            if (!forAll)
            {
                return;
            }
            ...
            foreach (BuildTargetGroup group in System.Enum.GetValues(typeof(BuildTargetGroup)))
            {
                if (BuildPipeline.IsBuildTargetSupported(group, defaultTarget(group)))
                    SetDefined(defineName, enable, NamedBuildTarget.FromBuildTargetGroup(group));
            }
        }
        ...
        public static void SetDefined(string defineName, bool enable, NamedBuildTarget group)
        {
        ...
        {
            return GetDefinesList(GetCurrentTarget());
        }

        public static List<string> GetDefinesList(NamedBuildTarget group)
        {
