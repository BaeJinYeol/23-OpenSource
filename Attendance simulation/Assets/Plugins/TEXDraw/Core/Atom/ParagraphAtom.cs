...
namespace TexDrawLib
{
    public sealed class ParagraphAtom : LayoutAtom
    {
        ...
        bool justified = false, righttoleft = false;
                ...
                if (!state.restricted)
                {
                    ...
                    if (i == children.Count && overflowedBox == null)
                    {
                        if (masterBox.children.Count == 0)
                        {
                            ...
                            if (righttoleft)
                            {
                                ListPool<Atom>.Release(children);
                            }
        ...
        HorizontalBox CheckBox(HorizontalBox b, bool useindent = false)
        {
        ...
        public override void ProcessParameters(string command, TexParserState state, string value, ref int position)
        {
            if (!state.Environment.current.IsInline())
            {
                ...
                righttoleft = state.Paragraph.rightToLeft;
