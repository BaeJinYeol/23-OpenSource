...
namespace TexDrawLib
{
    public class TabularCellAtom : BlockAtom
    {
        ...
        public override Box CreateBox(TexBoxingState state)
        {
            ...
            RuleBox.PutBorderVertical(hhbox, left.color, left.borderWidth, left.borderStyle, true, scale);
            RuleBox.PutBorderVertical(hhbox, right.color, right.borderWidth, right.borderStyle, false, scale);
            var vvbox = VerticalBox.Get(hhbox);
            RuleBox.PutBorderHorizontal(vvbox, top.color, top.borderWidth, top.borderStyle, true, scale);
            RuleBox.PutBorderHorizontal(vvbox, bottom.color, bottom.borderWidth, bottom.borderStyle, false, scale);
            if (background.a > 0)
            {
                hhbox.Add(0, StrutBox.Get(-hhbox.width, 0, 0));
                hhbox.Add(0, RuleBox.Get(background, hhbox.width, hhbox.height, hhbox.depth, scale));
            }
            return vvbox;
        }
